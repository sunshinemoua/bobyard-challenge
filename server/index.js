...
app.post("/comments", async (req, res) => {
  try {
    const { id, author, text, date, likes, image } = req.body || {};

    const query = `INSERT INTO comments (id, author, text, date, likes, image) VALUES (${id}, '${author}', '${text?.replace(
      /'/g,
      "''"
    )}', '${date}', ${likes}, '${image || ""}');`;
    await pool.query(query);
    const selectQuery = "SELECT * FROM comments ORDER BY id desc";
    const finalResults = await pool.query(selectQuery);
    return res.json(finalResults?.rows);

    // ADDING ALL ITEMS TO DB -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    //     const data = [
    //       {
    //         id: "1",
    //         author: "Joe",
    ...
    //       const { id, author, text, date, likes, image } = item || {};
    //       const query = `INSERT INTO comments (id, author, text, date, likes, image) VALUES (${id}, '${author}', '${text?.replace(
    //         /'/g,
    //         "''"
    //       )}', '${date}', ${likes}, '${image}');`;
...
app.post("/comments/:id", async (req, res) => {
  ...
  if (id) {
    const updateQuery = `UPDATE comments SET text = '${text}' WHERE id = ${id};`;
